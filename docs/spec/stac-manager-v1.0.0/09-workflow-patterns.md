...
## 5. Legacy Data Migration (Input File Pattern)
**Goal**: Generate STAC items from a static list of IDs, hydrating metadata from a separate CSV dump (Input File).

- **Source**: `SeedModule` (Emits bare IDs or skeleton items).
- **Processor**: `TransformModule` (Joins stream with CSV input data by ID).
...
steps:
  ...
  - id: hydrate_from_csv
    ...
    config:
      input_file: "./legacy/metadata_dump.csv"
      input_join_key: "id" # Matches STAC Item ID
      field_mapping:
        "properties.eo:cloud_cover": "Cloud_Cover"
        "properties.datetime": "Date_Acquired"
...
## 6. Deeply Nested JSON Migration (Complex Input File Pattern)
**Goal**: Generate STAC items using a complex, nested JSON input file (e.g., raw telemetry logs) using JMESPath queries to extract specific values.
...
steps:
  ...
  - id: hydrate_complex
    ...
    config:
      ...
      input_join_key: "meta.scene_id"
      
      field_mapping:
         # Extract nested dictionary value
         "properties.platform": "telemetry.platform"
         
         # Extract value from array of objects using filter
         "properties.optical_gain": "telemetry.sensors[?type=='optical'].gain | [0]"
         
         # Extract list of values
         "properties.eo:bands": "telemetry.sensors[0].bands"
         
         # Simple nested path
         "properties.datetime": "acquisition_meta.start_time"
