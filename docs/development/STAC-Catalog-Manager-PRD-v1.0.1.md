...
**Requirement**: Users can discover and list available Collections from a STAC API v1.0.0 endpoint.

**Inputs**:

- `catalog_url` (string): Base URL of STAC API v1.0.0 catalog endpoint
...
- List of Collection metadata (dict or JSON) with:
  - `id` (string): Collection identifier (per STAC v1.1.0)
...
**Requirement**: Users can fetch Items from a Collection via STAC API v1.0.0 with support for pagination and parallelization.
...
- Item data (list of GeoJSON features per STAC v1.1.0)
...
- Must handle STAC API v1.0.0 pagination (cursors, offsets)
...
**Requirement**: Users can create minimal valid STAC v1.1.0 Items from source data.
...
- Valid STAC v1.1.0 Item (dict/JSON) with:
  - `type` = "Feature"
  - `stac_version` = "1.1.0"
  - `id` (string)
  - `geometry` (GeoJSON)
  - `bbox` (list)
  - `properties` (dict with at least `datetime` or `start_datetime`/`end_datetime` per STAC v1.1.0)
...
- Items must pass STAC v1.1.0 JSON schema validation
...
- `metadata` (dict): STAC Item or Collection to extend (per STAC v1.1.0)
...
- Updated STAC metadata with:
  - Modified fields
  - `updated` timestamp (RFC 3339, per STAC v1.1.0 common metadata)
  - Audit trail in logs
...
- Must validate updated metadata against STAC v1.1.0 schema
...
- `metadata` (dict or list): STAC metadata (Items, Collections, Catalog per STAC v1.1.0)
- `output_format` (string): "json" or "parquet"
...
- Use `stac-validator` for core STAC v1.1.0 validation
...
- Support environment variable substitution (e.g., `${API_TOKEN}`)
...
- **JSON Schema**: Valid against STAC v1.1.0 JSON schemas
...
1. **Schema Validation**: All Items/Collections must validate against STAC v1.1.0 JSON schema
...
- ✅ Output Items pass STAC validation with `stac-validator` (v1.1.0 compliant)
...
- ✅ Output validates against STAC v1.1.0 + alternate-assets schemas
...
- ✅ All CSV records are converted to valid STAC v1.1.0 Items
- ✅ Custom fields are preserved in `properties`
- ✅ Geometry is correctly parsed from WKT
- ✅ Items pass STAC v1.1.0 validation
